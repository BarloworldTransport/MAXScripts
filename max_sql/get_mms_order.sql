...
SELECT id, xml FROM scheduledimportedtrip
WHERE xml LIKE "%ClientCode>MEAF%" AND xml LIKE "%Status>Delivered%" AND xml LIKE "%ShipmentNumber>PM%"
ORDER BY ID DESC
LIMIT 1;

# Look for order with shipment number
SELECT id FROM scheduledimportedtrip
WHERE xml LIKE "%ClientCode>MEAF%" AND xml LIKE "%Status>Accepted%" AND xml LIKE "%ShipmentNumber>PM967971%";
