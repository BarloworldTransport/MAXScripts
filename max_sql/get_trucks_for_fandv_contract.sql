SET @fandVContract_id = 4849; SET @objreg=(SELECT ID FROM objectregistry WHERE handle LIKE "udo%f%v%contract%truck%link%");
...
WHERE fvtl.fandVContract_id = @fandVContract_id ORDER BY tr.fleetnum ASC;
