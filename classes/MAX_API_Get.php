...
class MAX_API_Get
{
    ...
    public function runApiQuery()
    {
		...
		{
			$this->_addError('Run API Query', 'Caught Exception: ', $e->getMessage(), __FUNCTION__);
		}
    }
    ...
    public function __construct($_mode)
    {
        try {
            ...
            if (is_file($ini) === FALSE) {
				$this->_addError('Load INI File', 'INI File Not Found: ', $ini, __FUNCTION__);
                return FALSE;
            }
            ...
            if ((array_key_exists("apiuserpwd", $data) && $data["apiuserpwd"])) {
				...
				$this->_addError('Validate INI File Data', 'Required fields not found', 'Please check that apiuserpwd key=value is present in file: ' . $ini, __FUNCTION__);
                return FALSE;
            }
			...
			$this->_addError('object construct', '__construct failed with an Exception:', $e->getMessage(), __FUNCTION__);
            return FALSE;
        }
        // If code reache this point then code processed successfully
        return TRUE;
    }
    ...
    public function maxApiGetData()
    {
        ...
        if ($this->_apiFilter && $this->_apiObject && $this->_maxurl && $this->_apiuserpwd) {
            try {
				...
				$this->_addError('CURL fetch data', 'cURL failed to get data with Exception: ', $e->getMessage(), __FUNCTION__);
                return FALSE;
            }
            return $output;
        } else {
    ...
    private function splitResultIntoDataArray($_htmlResponse)
    {
        try {
            ...
            if (is_string($_htmlResponse) && $_htmlResponse && count($_httpUrlData) > 0) {
                ...
                if (is_array($_htmlRecPos) && count($_htmlRecPos) > 0) {
                ...
                $this->_addError('Check HTML Response', 'Not a valid HTTP response: ', $_htmlResponse, __FUNCTION__);
            }
            // : End
        } catch (Exception $e) {
            $this->_addError('Split Result Into Data Array', 'Caught Exception: ', $e->getMessage(), __FUNCTION__);
        }
    }
    ...
    private function addError($_step, $_errTitle, $_errDetail, $_errFunc)
    {
        if (is_string($_step) && is_string($_errTitle) && is_string($_errDetail) && is_string($_errFunc)) {
            ...
            if ($_errMsg && is_string($_errMsg)) {} else {
                return $_errMsg;
            }
        } else {
